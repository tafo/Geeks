    ...
    public class JumpSum
    {
        ...
        public static void RunCompareToLinq()
        {
            ...
            foreach (var test in tests)
            {
                ...
                var indexes = test[1].Split(' ').Select((x, i) => new {x = int.Parse(x), i}).OrderBy(x => x.x).Select(x => x.i).ToArray();
                Console.WriteLine(indexes.Skip(1).Select((x, a) => Math.Abs(x - indexes[a])).Sum());
            }
        }
